...
def plot_rotation(cloud_A, cloud_B, pos_A, pos_B, dir_A, dir_B, cent_A, cent_B, cent_pos, plane_cent_A, plane_cent_B, aligned_plane_cent_B, rotated_aligned_cent_rot, angles, error, rotation_angle):
    ...
    dir_A_color = get_dir_color(dir_A)
    dir_B_color = get_dir_color(dir_B)
    ax = plt.subplot(3,3,4, projection = '3d')
    ax.scatter(*cloud_A[:,:3].T, color = dir_A_color, s= 10)
    ax.scatter(*cloud_B[:,:3].T, color = dir_B_color, s= 10)
    process_axis(ax)
    ax.scatter([], [], color=[14/255, 14/255, 143/255], label='none')
    ax.scatter([], [], color=[12/255, 136/255, 249/255], label='left')
    ax.scatter([], [], color=[17/255, 219/255, 224/255], label='right')
    ax.legend()
    ...
    pa = ax.scatter(*cloud_A[:,:3].T, c = pos_A[:,0], s= 10, cmap = 'viridis')
    pb = ax.scatter(*cloud_B[:,:3].T, c = pos_B[:,0], s= 10, cmap = 'magma')
    process_axis(ax)

    # Increase the horizontal displacement of the colorbars
    ax_pos = ax.get_position()  # Get the position of the third plot
    colorbar_height = (ax_pos.y1 - ax_pos.y0)*0.7
    colorbar_center = (ax_pos.y0 + ax_pos.y1) / 2
    cba = fig.add_axes([ax_pos.x1 + 0.03, colorbar_center - colorbar_height / 2, 0.01, colorbar_height])
    plt.colorbar(pa, cax=cba, orientation='vertical')
    cba.set_ylabel('pre position')

    cbb = fig.add_axes([ax_pos.x1 + 0.08, colorbar_center - colorbar_height / 2, 0.01, colorbar_height])
